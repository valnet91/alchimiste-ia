...
def render_readme(site: dict[str, str], entries: list[dict[str, str]]) -> str:
    lines = [
        ...
        "",
        f"> {site['tagline']}",
        f"> {site['site_url']} · {site['github_repo']} · RNA {site['rna']}",
        "",
        f"**[Voir et entendre les 10 contrôles]({site['pages_url']})** — cartes + voix Henri / Denise.",
        "",
        "## Scanner",
        "",
...
def render_llms(site: dict[str, str], entries: list[dict[str, str]]) -> str:
    lines = [
        ...
        f"- Cartes parlantes: {site['pages_url']}",
        f"- Dépôt: {site['github_repo']}",
        "",
        "## Dix contrôles",
        "",
    ]
